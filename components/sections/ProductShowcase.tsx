...
export default function ProductShowcase({
  ...
  return (
    <div
      id="product-showcase"
      className={`${isLoggedIn ? "min-h-screen h-auto overflow-auto" : "h-screen overflow-hidden"} bg-transparent text-white py-12 px-4 relative flex flex-col justify-center`}
    >
              ...
              {filteredProducts.length > 0
                ...
                : /* Placeholder/Loading skeleton items */
                  Array.from({ length: 4 }).map((_, i) => (
                    <div
                      key={i}
                      className="group relative rounded-3xl overflow-hidden block aspect-square bg-white/5 animate-pulse"
                    />
                  ))}
            </div>
